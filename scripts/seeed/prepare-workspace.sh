...
Options:
  ...
  --module-sku SKU  AGX Orin module SKU, for example 0004
...
if is_agx_orin_machine "$MACHINE"; then
    supported_skus=$(supported_module_skus "$MACHINE")
    if [[ -z $MODULE_SKU ]]; then
        cat >&2 <<EOF
ERROR: --module-sku is required for AGX Orin machine $MACHINE.
...
EOF
    ...
    echo "ERROR: --module-sku is currently supported only for AGX Orin machines." >&2
    exit 2
fi
...
if [[ -f $BUILD_DIR/conf/local.conf ]]; then
    ...
    if is_agx_orin_machine "$MACHINE"; then
        ...
        if [[ -z $configured_module_sku ]]; then
            cat >&2 <<EOF
ERROR: existing AGX Orin build directory has no explicit module SKU.
  Build dir: $BUILD_DIR
  Machine:   $MACHINE

Use a new --build-dir for module SKU $MODULE_SKU. This prevents mixing
artifacts generated for different AGX Orin modules.
EOF
            exit 1
        fi
